...
while [[ $# -gt 0 ]]; do
  case $1 in
    --help|-h)
      echo "Usage: ./scripts/build-vsix.sh [--vsix-name <name>] [--vsix-version <version>] [--output-dir <dir>]"
      echo ""
      echo "Flags:"
      echo "  --vsix-name <name>        VSIX name (default: openchamber)"
      echo "  --vsix-version <version>  Override version (default: <pkgjson-version>-local-<short-sha>)"
      echo "  --output-dir, --output <dir>  Output directory (default: <repo>/dist)"
...
# Build VSIX using @vscode/vsce (the maintained CLI; the bare `vsce` package is deprecated)
# Prefer the local @vscode/vsce bin from packages/vscode's devDependencies, then npx/bunx.
LOCAL_VSCE="$REPO_ROOT/packages/vscode/node_modules/.bin/vsce"
if [ -x "$LOCAL_VSCE" ]; then
  VSX_EXEC="$LOCAL_VSCE"
elif npx --yes @vscode/vsce --version &>/dev/null; then
  VSX_EXEC="npx --yes @vscode/vsce"
elif bunx --yes @vscode/vsce --version &>/dev/null; then
  VSX_EXEC="bunx --yes @vscode/vsce"
else
  echo "   [Warn] Could not resolve @vscode/vsce (checked local bin, npx, bunx)" >&2
  exit 1
fi
